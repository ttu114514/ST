...
class ODESolve:
    ...
    def invert_i2n2i(self, prompt1, prompt2, image_start_latent, flip=False, flip_mask=None):
        # 怎麼用
        # image_start, latent_start, image_end, latent_end = ode.invert_i2n2i(prompt_1, prompt_2, image_gt_latent, flip=False)
        self.init_prompt(prompt1)
        latent_i2n = self.ddim_loop(image_start_latent, is_forward=True)
        xT = latent_i2n[-1]
        # print("Final latent range:", xT.min(), xT.max())

        if flip:
            # xT= torch.flip(xT, dims=[2])
            #=====================================================================================================
            if flip_mask is None:
                # flip_mask = torch.randint(0, 2, xT.shape[-2:], device=xT.device).float()  # 生成随机掩码
#                 # 先創建一個全為 0 的掩碼
                flip_mask = torch.zeros(xT.shape[-2:], device=xT.device).float()

#                 # 隨機選擇 512 個位置設置為 1
                ratia = int(512)
                indices = torch.randperm(flip_mask.numel(), device=xT.device)[:ratia]  # 隨機選擇512個索引
                flip_mask.view(-1)[indices] = 1  # 將這些位置設置為1
                
                # xT_flipped = torch.tanh(xT)  # 水平翻转
            xT_flipped = -xT

            # 在需要翻转的部分应用翻转操作
            # xT_flipped = torch.flip(xT, dims=[2])  # 水平翻转
            
            # 使用掩码选择要翻转和不翻转的部分
            xT = flip_mask.unsqueeze(0).unsqueeze(0) * xT_flipped + (1 - flip_mask.unsqueeze(0).unsqueeze(0)) * xT
            # xT = [torch.where(flip_mask==1, -xT , xT) for i in range(2)]
            #=====================================================================================================

        self.init_prompt(prompt2)
        latent_n2i = self.ddim_loop(xT, is_forward=False)

        return self.latent2image(image_start_latent), image_start_latent, self.latent2image(latent_n2i[-1]), latent_n2i[-1], flip_mask


if __name__ == "__main__":

    parser = argparse.ArgumentParser()
    parser.add_argument('--image_path', type=str, default='./asserts/001.png', help='test image path')
    parser.add_argument('--private_key', type=str, default='the eiffel tower', help='text prompt of the private key')
    parser.add_argument('--public_key', type=str, default='a tree', help='text prompt of the public key')
    parser.add_argument('--save_path', type=str, default='./output/flip/001', help='text prompt of the public key')
    ...
    _, _, _, image_hide_latent, flipmask = ode.invert_i2n2i(prompt_1, prompt_2, image_gt_latent, flip=True)
    ...
    _, _, _, image_reverse_latent, _ = ode.invert_i2n2i(prompt_2, prompt_1, image_hide_latent_reveal, flip=True, flip_mask=flipmask)
    ...
    # img1 = cv2.imread("{:s}/gt.png".format(args.save_path))
    # img2 = cv2.imread("{:s}/reverse.png".format(args.save_path))
    # print("PSNR：", cv2.PSNR(img1, img2))
    print("PSNR：", cv2.PSNR(image_gt, image_reverse))
